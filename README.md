...
**إذا كنت قد ثبّت schema.sql مسبقاً وتحتاج تحديث فقط**:

1. من Supabase Dashboard → SQL Editor → New query
2. الصق محتوى `supabase/migration-v2.sql` واضغط Run
3. هذا التحديث يزيل قيد النوع (يسمح بكتابة أنواع مواعيد مخصصة) ويحدّث اسم المستخدم إلى "الحبيب".
